...
struct BoneInfo
{
    FString Name;
    int index;
    TArray<WraithAnimFrame<FVector>> BonePositions;
    TArray<WraithAnimFrame<FQuat>> BoneRotations;
    TArray<WraithAnimFrame<FVector>> BoneScale;

    FVector GetPositionAtFrame(uint32_t FrameAsked) const
    {
    ...
    FQuat GetRotationAtFrame(uint32_t FrameAsked) const
    {
        ...
        return FQuat(-1, -1, -1, -1);
    }

    FVector GetScaleAtFrame(uint32_t FrameAsked) const
    {
...
public:
    ...
    static FQuat FixRotationAbsolute(FQuat QuatRot, FQuat InitialRot);
    static FVector FixPositionAbsolute(FVector QuatPos, FVector InitialPos);
};
